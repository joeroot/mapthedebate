...
# File paths
$lexpath   = File.join(File.dirname(__FILE__), '../data/pos')
...
class TweetTagger
  ...
  def self.general pos
    GENERAL.select{|k,v| pos =~ v}.keys.first.to_s
  end
  ...
  def initialize(params = {})
    ...
    $additional_tags.each {|tag| self.load_tags tag}
    $additional_words.each {|word| self.load_words word}
  end
  ...
  def fetch_tags(text, verbose = false)
    return nil unless valid_text(text)
    tagged = []
    words = clean_text(text)
    tags = Array.new
  ...
  def split_punct(text)
    ...
    text = text.gsub(/(?<!http):/o, " :") 
    # text = text.gsub(/:/o, " :") # Shift semicolons off
    # text = text.gsub(/http :/o, "http:") # fixes http
    # text = text.gsub(/https :/o, "https:") # fixed https
    text = text.gsub(/(\.\.\.+)/o){" " + $1 + " "} # Shift ellipses off 
    text = text.gsub(/([\(\[\{\}\]\)])/o){" " + $1 + " "} # Shift off brackets
    # originally container #s and was /([\!\?#\$%;~|])/o
  ...
  def clean_word(word)
    word = word[0..-2] if word.last == ":" and word.length > 1
    lcf = lcfirst(word)
    ucf = ucfirst(word)
    ...
    if @@lexicon[word]
      ...
      return lcf
    elsif @@lexicon[ucf] and @@lexicon[ucf]["nnp"]
      # if the word is likely to be a proper noun e.g. a name
      return ucf
    else
